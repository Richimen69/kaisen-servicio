...
function ClientLayout() {
  // Buscamos si el técnico está compartiendo su cámara en tiempo real
  const cameraTracks = useTracks([{ source: Track.Source.Camera, withPlaceholder: false }]);
  const videoTrack = cameraTracks[0]; // Primera pista de video encontrada en la sala
  
  // Hook para controlar el micrófono local del cliente
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-950">
      ...
      <div className="flex-1 relative flex items-center justify-center bg-slate-950 overflow-hidden">
        {videoTrack ? (
          // Si el técnico tiene su cámara encendida, mostramos el video completo
          <VideoTrack trackRef={videoTrack} className="w-full h-full" />
        ) : (
          // Si el técnico pausó la cámara virtual, mostramos tu estado de pausa
